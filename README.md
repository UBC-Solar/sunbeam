...
TimescaleDB/Postgres database — while a broker server orchestrates worker
...
and schedules each subgraph on its own thread. Stages that consume signals
...
non-blocking writer into `aligned_sample` in Postgres. A worker can run
standalone (`--serverless`), or report in to the **broker** — a FastAPI
server (`server/`) that can launch workers as Docker containers, supervise
...
| [`docs/API.md`](docs/API.md) | The full broker HTTP/SSE API: events, workers, pipeline editions, and the telemetry query + streaming endpoints. |
...
This brings up Postgres/TimescaleDB, the broker (`http://localhost:8000`,
...
orchestration/  Worker<->broker protocol: WorkerControl, OrchestratorClient, bootstrap
db/           SQLAlchemy models, event/aligned-sample writers, InfluxDB readers
server/       The FastAPI broker: routes, services, the debug stream viewer
...
for what each extra/group is for and why `broker`/`executor` and
